...
- **优先级**：按战斗价值降序分配（复用 `CombatEvaluator.ComputeCombatValue`）
- **全局保底**：若全局无任何殖民者穿戴消防背包，强制最高战斗价值纯近战角色拿消防背包
- **评分**：护盾腰带仅对重甲前排（Heavy）+100，自由后排（Flexible）/轻甲工人（Light）0 分不入选；消防背包对所有纯近战角色 +60；品质 ×5 加分

| 腰带类型 | 评分 | 适用对象 |
|---------|------|---------|
| 护盾腰带 | +100（仅 Heavy） | 重甲前排（Brawler），贴身近战免疫远程射击；Flexible/Light 不分配 |
| 消防背包 | +60 | 所有纯近战角色，应对火灾/机械族，且允许配远程/EMP 副武器 |
