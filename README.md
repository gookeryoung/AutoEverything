...
| **S** | 1. 乱开枪 + 射击单火<br>2. 坚韧 + 格斗有火（Minor 或 Major）<br>3. 工作狂 + 神经质 + 1 个专业工作双火<br>4. 拥有任一特殊天赋特质：博闻强识（TooSmart）/开心果（Joyous）/极致体能（BodyMastery）/痴迷虚空（VoidFascination）/神秘学者（Occultist）/怪诞不经（Disturbing）<br>5. 沉鱼落雁（Beauty degree=2）+ 社交双火 | 全局高价值 |
...
- **Harmony 补丁降级**：`PawnUIOverlay` 类型或 `DrawPawnGUIOverlay` 方法缺失时仅 `Log.Warning`，星标不显示但不崩溃。RimWorld 1.6 中 `PawnUIOverlay` 位于 `Verse` 命名空间（非 `RimWorld`），补丁加载时优先用 `Verse.PawnUIOverlay`，回退 `RimWorld.PawnUIOverlay` 兼容旧版本
...
| `GearAllocator` | `ComputeHeavyUpgradeFlags` | `int heavyArmorCount, int heavyPawnCount, List<ArmorPreference>` | 重甲顺延名额计算 |
| `GearAllocator` | `ShouldStealFromWearerCore` | `float stealerScore, float wearerScore, float threshold` | 扒装守卫得分比较（防振荡） |
| `GearScorer` | `ComputeArmorScoreCore` | `float sharp, float blunt, float heat, float weight` | 护甲值得分 |
| `GearScorer` | `ComputeLayerMatchScoreCore` | `ArmorPreference, float[] layerWeights` | 定位契合适配得分 |
| `GearScorer` | `ComputeInsulationScoreCore` | `float insulation, float weight, bool strict` | 保暖隔热得分 |
| `GearScorer` | `ComputeMovementPenaltyCore` | `float moveSpeedFactor, float sensitivityWeight` | 移动减损得分 |
...
当前覆盖：391 个用例（含 `ApplySkillFloorCoreTests` 30 / `EvaluateAutoTierCoreTests` 32 / `PawnMarkerTests` 101 / `GearAllocatorTests` 119 / `GearScorerTests` 109）。
