...
面板底部"全局人物评级"按钮提供一键为所有殖民者 Nick 加上评级前缀的功能：

| 操作 | 效果 |
|------|------|
| 应用评级到名字 | 所有殖民者 Nick 变为 `S#王五` `A#李四` 格式，玩家一眼分辨评级 |
| 清除评级标签 | 恢复原 Nick，从字典取原名或按前缀解析剥离 |

**覆盖范围**：殖民者 + 食尸鬼（Anomaly DLC）。食尸鬼也按相同规则评级，但不参与装备分配——玩家可一眼分辨其价值。
...
**持久化**：原名字典不存档，游戏退出后丢失；但 Nick 本身持久化到 RimWorld 存档，重启后清除仍可按前缀解析剥离恢复。
