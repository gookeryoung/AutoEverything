...
| 移动减损 | `(Mass / 5.0) × 角色敏感度权重` | 归一化 mass 到 0~1（RimWorld apparel mass 最大约 5.0kg），与护甲值得分量级匹配；工人 `geWorkerMovePenaltyWeight`、后排 `geBackRowMovePenaltyWeight`、前排 `geFrontRowMovePenaltyWeight` |
...
| `GearScorer` | `ComputeMovementPenaltyCore` | `float mass, Role role, float workerW, float backRowW, float frontRowW` | 移动减损得分（mass 归一化到 0~1） |
